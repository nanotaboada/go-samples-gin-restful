...
- `docs/adr/0012-pure-go-sqlite-driver.md`: ADR documenting the migration from the CGO-based `mattn/go-sqlite3` driver to the pure-Go `github.com/glebarez/sqlite` wrapper backed by `modernc.org/sqlite` (#237)
...
- `data/player_data.go`: replaced `gorm.io/driver/sqlite` import with `github.com/glebarez/sqlite`; driver swap is a one-line change — `gorm.Open(sqlite.Open(...))` call and DSN formats are unchanged (#237)
- `Dockerfile`: removed `gcc`, `musl-dev`, `sqlite-dev` from the builder stage; set `CGO_ENABLED=0` — builder no longer requires a C toolchain (#237)
- `docs/adr/0011-docker-and-compose-strategy.md`: marked the CGO revisit trigger as resolved, cross-referencing ADR-0012 (#237)
