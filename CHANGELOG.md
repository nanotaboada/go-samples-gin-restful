...
- `GET /players/uuid/:id` endpoint for UUID-based lookup (surrogate key, internal); consistent with `GET /players/{player_id}` in the Python sibling project and `GET /{id:Guid}` in .NET; the `/uuid/` static prefix avoids ambiguity with `/players/squadnumber/:squadnumber`
...
- **BREAKING** `GET /players/:id` split into `GET /players/squadnumber/:squadnumber` (user-facing, by squad number) and `GET /players/uuid/:id` (by internal UUID)
