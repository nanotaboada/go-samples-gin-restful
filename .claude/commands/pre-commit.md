...
2. Run `go fmt ./...`
3. Run `go mod tidy`. If `go.mod` or `go.sum` changed, stop and report it —
   propose committing the tidy result first and wait for my explicit
...
9. If `coderabbit` CLI is installed, run `coderabbit review --type uncommitted --agent`:
